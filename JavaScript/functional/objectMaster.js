...
const moreType = pokémon.map(p => p.name);
console.log(moreType);
//  an array with just the names of pokémon with an id greater than 99
const nameGre = pokémon.filter(p => p.id > 99).map(p => p.name);
console.log(nameGre);
//  an array with just the names of the pokémon whose only type is poison
const nameJust = pokémon.filter(p => p.types == "poison").map(p => p.name);
console.log(nameJust);
//  an array containing just the first type of all the pokémon whose second type is "flying"
const secondType = pokémon.filter(p => p.types[1] == "flying").map(p => p.types[0]);
console.log(secondType);
//  a count of the number of pokémon that are "normal" type
const normalType = pokémon.filter(p => p.types == "normal").length;
